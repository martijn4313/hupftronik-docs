...
your firmware and wiring budget support the extra input, since it unlocks sequential fueling and
individual-cylinder ignition timing rather than forcing batch fire and wasted spark.

!!! warning "Hall sensors are polarity-sensitive"
    The cam sensor has a supply, ground, and signal pin, unlike the passive crank sensor. Verify
    pinout against your factory wiring diagram before connecting power — reversed supply and ground
    can damage the sensor.

### 1.3. Design Rationale

The same reasoning as the VW 1.8T applies: retaining the cam sensor costs one input but buys
sequential capability that's particularly valuable on a high-output, knock-sensitive variant like the
Clio Sport engines.
...
stage — wire the $+5\,\text{V}$ logic-level trigger from the 24P V1 directly to each coil, and
confirm trigger polarity against the coil's datasheet before connecting all four. Earlier or lower
trim variants may use paired coils (wasted spark) instead — verify which architecture your specific
car has before wiring.
...
| Ignition mode | Coil-on-plug, sequential or wasted spark depending on variant |
