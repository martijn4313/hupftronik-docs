...
import { simulate, SIM_SOURCES } from './sim.js';
...
export function addComp(type){
  ...
  } else if(type==='schildknappe'){
    c.pins=d.getPins(c.ioCount);
  } else if(d.getPins){
...
export function setupSVGHandlers(){
  ...
  if(pin){
    ...
    /* completing a pending wire on a pin click is handled by the
       state.pending&&pin branch above (it always returns), so
       state.pending is guaranteed false here: this always starts a new
       pending wire */
    state.pending={compId,pinId}; state.pendingWp=[]; state.connectCandidate=null; clearTrace();
    render();
    return;
  }
  if(noteHandle){
